import { computed, ref, type ShallowReactive, shallowReactive, shallowRef } from 'vue'
...
export const useStore = defineStore('store', () => {
    ...
    const projects = shallowReactive<Record<string, ShallowReactive<Project>>>({})
    ...
    const updateData = (itemType: 'node' | 'edge', models: Node | Edge | Node[] | Edge[]) => {
      ...
      (Array.isArray(models) ? models : [models]).forEach(item => {
        ...
        if (itemType === 'edge') {
          ...
          project.bothEdgesMap.get(oldSource)?.delete(oldEdge)
          project.bothEdgesMap.get(oldTarget)?.delete(oldEdge)

          if (newSource in project.bothEdgesMap) {
            project.bothEdgesMap.get(newSource).add(oldEdge)
          } else {
            project.bothEdgesMap.set(newSource, new Set([oldEdge]))
          }

          if (newTarget in project.bothEdgesMap) {
            project.bothEdgesMap.get(newTarget).add(oldEdge)
          } else {
            project.bothEdgesMap.set(newTarget, new Set([oldEdge]))
          }
        }
      })
    ...
    const addData = (itemType: 'node' | 'edge', models: Node | Edge | Node[] | Edge[]) => {
      ...
      (Array.isArray(models) ? models : [models]).forEach(item => {
          ...
          if (itemType === 'edge') {
            ...
            if (edge.source in project.bothEdgesMap) {
              project.bothEdgesMap.get(edge.source).add(edge)
            } else {
              project.bothEdgesMap.set(edge.source, new Set([edge]))
            }

            if (edge.target in project.bothEdgesMap) {
              project.bothEdgesMap.get(edge.target).add(edge)
            } else {
              project.bothEdgesMap.set(edge.target, new Set([edge]))
            }
          }
        }
      )
    ...
    const removeData = (itemType: 'node' | 'edge', id: ID | ID[]) => {
      ...
      (Array.isArray(id) ? id : [id]).forEach(item => {
        if (itemType === 'node') {
          project.bothEdgesMap.get(item).forEach(edge => project.edgesMap.delete(edge.id))
          project.nodesMap.delete(item)
          project.inEdgesMap.delete(item)
          project.outEdgesMap.delete(item)
          project.bothEdgesMap.delete(item)
        }
        if (itemType === 'edge') {
          ...
          project.bothEdgesMap.get(source)?.delete(edge)
          project.bothEdgesMap.get(target)?.delete(edge)
        }
      })
    ...
    const addProject = (project: Project) => {
      projects[project.id] = shallowReactive(project)
      return project.id
    }
