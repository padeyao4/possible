...
import { CARD_WIDTH, days, usePlanStore } from '@/stores';
...
function handleChange() {
  if (viewModel.inputValue) {
    const offsetX = days(new Date(viewModel.inputValue)) * CARD_WIDTH;
...
<template>
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="calendar-icon" @click="handleCalendar">
  ...
  <input ref="dateInputRef" v-model="viewModel.inputValue" class="date-select" orientation="bottom" type="date"
    @change="handleChange" />
</template>
